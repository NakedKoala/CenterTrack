...
class DLA(nn.Module):
    def __init__(self, levels, channels, num_classes=1000,
        ...
        self.raft_level0 = self._make_conv_level(
            channels[0], channels[0], levels[0])
        self.raft_level1 = self._make_conv_level(
            channels[0], channels[1], levels[1], stride=2)
        self.raft_level2 = Tree(levels[2], block, channels[1], channels[2], 2,
                           level_root=False,
                           root_residual=residual_root)
        self.raft_level3 = Tree(levels[3], block, channels[2], channels[3], 2,
                           level_root=True, root_residual=residual_root)
        self.raft_level4 = Tree(levels[4], block, channels[3], channels[4], 2,
                           level_root=True, root_residual=residual_root)
        self.raft_level5 = Tree(levels[5], block, channels[4], channels[5], 2,
                           level_root=True, root_residual=residual_root)
    ...
    def forward(self, x, pre_img=None, pre_hm=None, raft=None):
        ...
        for i in range(6):
            ...
            raft_repr = getattr(self, 'raft_level{}'.format(i))(raft_repr)
